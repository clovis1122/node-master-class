...
orders.create = async function(req) {
  ...
  const { creditCard } = req.parsedUrl.query;
  ...
  if (!validCreditCard) {
    return { statusCode: 400, data: 'Invalid credit card.' };
  }
  ...
  try {
    ...
    return { statusCode: 400, data: 'Could not obtain the user shopping cart.' };
  }
  ...
  try {
    ...
    try {
      ...
      return { statusCode: 500, data: 'Unable to retrieve or create the user orders.' };
    }
    userOrders = [];
  }
  ...
  try {
    ...
    return { statusCode: 500, data: 'Unable to read the menu of items.' };
  }

  // Creates the new user order and adds it to the user orders.
  const userOrder = { items: userCart.map(itemId => menuItems[itemId]) };
  ...
  try {
    ...
    return { statusCode: 500, data: 'Unable to process the payment.' };
  }
  ...
  try {
    ...
    return { statusCode: 500, data: 'Unable to complete the user order creation.' };
  }
