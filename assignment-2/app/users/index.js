...
users.create = async function(req) {
  ...
  if (!name || !email || !address || !password) {
    return { statusCode: 400, data: 'Missing required fields.' };
  }
...
users.update = async function(req) {
  ...
  if (!hasOneEditParam) {
    return { statusCode: 400, data: 'Missing field to update.' };
  }
  ...
  try {
    ...
    return { statusCode: 400, data: 'Could not find the user to update.' };
  }
...
users.delete = async function(req) {
  ...
  if (!email) {
    return { statusCode: 400, data: 'Missing the email.' };
  }

  try {
    await await users._db.delete('users', email);
    return { statusCode: 204, data: 'User deleted successfully.'};
  } catch(e) {
    return { statusCode: 500, data: 'Could not delete the user.' };
  }
}
